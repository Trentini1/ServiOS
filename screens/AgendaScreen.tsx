...
export default function AgendaScreen({ onVoltar, onAbrirOS }: Props) {
  ...
  const marcadores = ordens.reduce<Record<string, { dots: { color: string; key: string }[] }>>(
    (acc, o) => {
  ...
  if (marcadores[dataSelecionada]) {
    (marcadores[dataSelecionada] as any).selected = true;
    (marcadores[dataSelecionada] as any).selectedColor = '#2563eb33';
  } else {
    marcadores[dataSelecionada] = { dots: [], selected: true, selectedColor: '#2563eb33' } as any;
  }
          ...
          <>
            ...
            {ordensNaData.length === 0 && (
              <View style={styles.vazio}>
                <Ionicons name="calendar-clear-outline" size={36} color="#334155" />
            ...
            <View style={styles.cardConteudo}>
              <View style={styles.cardTopo}>
                ...
                <View style={[styles.badge, { backgroundColor: CORES_STATUS[item.status] + '22' }]}>
                  <Text style={[styles.badgeTexto, { color: CORES_STATUS[item.status] }]}>
                    {item.status}
                  </Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  secaoTitulo: { color: '#94a3b8', fontSize: 13, fontWeight: '600' },
  lista: { paddingBottom: 40 },
  vazio: { alignItems: 'center', paddingTop: 20, gap: 8 },
  vazioTexto: { color: '#475569', fontSize: 13 },
  card: {
    flexDirection: 'row',
    marginHorizontal: 16,
    marginBottom: 10,
  ...
  cardCliente: { color: '#ffffff', fontSize: 14, fontWeight: '600', flex: 1, marginRight: 8 },
  badge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  badgeTexto: { fontSize: 11, fontWeight: '700' },
  cardDetalhe: { color: '#64748b', fontSize: 12, marginTop: 2 },
  tecnicoRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 6 },
  tecnicoTexto: { color: '#64748b', fontSize: 12 },
});
