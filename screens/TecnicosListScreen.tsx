...
import {
  View, Text, StyleSheet, TouchableOpacity, FlatList, RefreshControl, Alert,
} from 'react-native';
...
export default function TecnicosListScreen({ onVoltar, onNovoTecnico }: Props) {
  ...
  function confirmarExclusao(tecnico: Tecnico) {
    Alert.alert('Excluir Técnico', `Deseja remover ${tecnico.nome}?`, [
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.vazio}>
            ...
            <Text style={styles.vazioTexto}>Toque no botão abaixo para cadastrar o primeiro técnico.</Text>
          </View>
        }
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <TouchableOpacity onPress={() => confirmarExclusao(item)} style={styles.excluirBtn}
...
function criarEstilos(t: AppTema) {
  return StyleSheet.create({
    ...
    card: {
      ...
      borderRadius: 14, padding: 14, borderWidth: 1, borderColor: t.borda, marginBottom: 10,
    },
    cardIcone: {
      width: 42, height: 42, borderRadius: 11, backgroundColor: '#9333ea22',
      alignItems: 'center', justifyContent: 'center', marginRight: 12,
    },
    cardInfo: { flex: 1 },
    cardNome: { color: t.texto, fontSize: 15, fontWeight: '600' },
    cardCargo: { color: t.textoMuted, fontSize: 12, marginTop: 2 },
    telefoneRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 },
    cardTelefone: { color: t.textoMuted, fontSize: 12 },
    excluirBtn: {
    ...
    fab: {
      ...
      width: 56, height: 56, borderRadius: 28, backgroundColor: '#9333ea',
      alignItems: 'center', justifyContent: 'center',
